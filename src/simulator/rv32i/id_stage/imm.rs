...
mod tests {
    ...
    fn test_imm() {
        ...
        let mut opcode = 0b001_0011;
        let mut inst = 0b00000000001000001000000100000011;
        tb.connect(constant.alloc(ConstsAlloc::Out(opcode)), Connect::Opcode);
        tb.connect(constant.alloc(ConstsAlloc::Out(inst)), Connect::Inst);
        assert_eq!(imm.read(), 2);
        // S-type: sw x1, -4(x2)
        // 1111111 | 00010 | 00001 | 010 | 11100 | 0100011
        opcode = 0b010_0011;
        inst = 0b11111110001000001010111000100011;
        tb.connect(constant.alloc(ConstsAlloc::Out(opcode)), Connect::Opcode);
        tb.connect(constant.alloc(ConstsAlloc::Out(inst)), Connect::Inst);
        ...
        opcode = 0b010_0011;
        inst = 0b00011010100000010010011000100011;
        tb.connect(constant.alloc(ConstsAlloc::Out(opcode)), Connect::Opcode);
        tb.connect(constant.alloc(ConstsAlloc::Out(inst)), Connect::Inst);
        ...
        opcode = 0b110_1111;
        inst = 0b00110100011000000010000011101111;
        tb.connect(constant.alloc(ConstsAlloc::Out(opcode)), Connect::Opcode);
        tb.connect(constant.alloc(ConstsAlloc::Out(inst)), Connect::Inst);
